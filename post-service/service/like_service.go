...
// ToggleLike 点赞/取消点赞切换（post-service 仅处理 post / post_comment）
...
func incrementLikes(targetType string, targetID uint) (int, error) {
	switch targetType {
	case "post":
		if err := config.DB.Model(&model.Post{}).Where("id = ?", targetID).
			Update("likes", clause.Expr{SQL: "likes + 1"}).Error; err != nil {
			return 0, err
		}
		var post model.Post
		config.DB.First(&post, targetID)
		return post.Likes, nil
	case "post_comment":
		if err := config.DB.Model(&model.CommentPost{}).Where("id = ?", targetID).
			Update("likes", clause.Expr{SQL: "likes + 1"}).Error; err != nil {
			return 0, err
		}
		var c model.CommentPost
		config.DB.First(&c, targetID)
		return c.Likes, nil
	default:
		return 0, errors.New("unknown target type for post-service: " + targetType)
	}
}
