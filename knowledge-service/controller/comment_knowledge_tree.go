// controller/comment_knowledge_tree.go
package controller

import "rxtcloud/knowledge-service/model"

// BuildCommentKnowledgeTree 将知识评论列表转换为树形结构
func BuildCommentKnowledgeTree(comments []model.CommentKnowledge) []map[string]interface{} {
	// 按 level 分组
	var level1 []model.CommentKnowledge
	var level2 []model.CommentKnowledge
	var level3 []model.CommentKnowledge
	...
	// 构建 level 2 的映射
	level2Map := make(map[uint][]model.CommentKnowledge)
	for _, c := range level2 {
		level2Map[c.ParentID] = append(level2Map[c.ParentID], c)
	}

	// 构建 level 3 的映射
	level3Map := make(map[uint][]model.CommentKnowledge)
	for _, c := range level3 {
		level3Map[c.ParentID] = append(level3Map[c.ParentID], c)
	}

	// 构建评论项
	buildCommentItem := func(c model.CommentKnowledge) map[string]interface{} {
