...
var (
	knowledgeSensitiveWords []string
	knowledgeWordMutex      sync.RWMutex
)
...
func InitSensitiveWords() error {
	...
	knowledgeWordMutex.Lock()
	defer knowledgeWordMutex.Unlock()

	knowledgeSensitiveWords = make([]string, 0, len(words))
	for _, w := range words {
		knowledgeSensitiveWords = append(knowledgeSensitiveWords, w.Word)
	}
	return nil
}
...
func HasSensitiveWord(text string) bool {
	...
	knowledgeWordMutex.RLock()
	defer knowledgeWordMutex.RUnlock()
	text = strings.ToLower(text)
	for _, word := range knowledgeSensitiveWords {
